    ...
    <nav className="bg-white px-4 py-2.5 sm:py-2 fixed w-full z-20 top-0 left-0 border-b border-[#dbdbdb] dark:border-[#333] dark:bg-[#111]">
      <div className="container flex flex-wrap items-center justify-between mx-auto max-w-xl">
        ...
        <div className="flex md:order-2">
          <div onClick={() => handleThemeSwitch()} className='flex items-center justify-center mr-5 cursor-pointer sm:hidden'>
            {theme === 'light' ?
          ...
          <Link to='/account' className="flex items-center mr-5 sm:hidden">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-zinc-600 dark:bg-[#111] dark:text-white">
          ...
          <button onClick={handleCreatePost} type="button" className="text-black bg-white hover:bg-gray-100 font-medium rounded-full text-sm px-5 py-2 text-center mr-3 md:mr-0 border dark:bg-black dark:border-[#333] dark:text-white dark:hover:bg-[#111]">Create</button>
        </div>
